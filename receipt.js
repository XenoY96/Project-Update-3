...
(function(){
  ...
  // Right before a snapshot, forces every receipt line to its fully-
  // settled end state via inline opacity — this is the fix for the
  // download (and now, apparently, the live view too) coming out
  // washed-out/blank: html2canvas can catch an opacity transition before
  // the browser has actually finished painting it, even though the live
  // page already looks correct by that point. (This used to also force
  // `filter`, but the print-in blur effect itself turned out to be the
  // deeper problem — html2canvas doesn't reliably render elements that
  // use CSS filter at all, so it's been dropped from receipt.css.)
  function rtForceSettledState(paper){
    paper.querySelectorAll('.rt-ink').forEach(el => {
      el.style.opacity = '1';
    });
    const stampRing = document.querySelector('#rtStamp .rt-stamp-ring');
    if(stampRing){
      stampRing.style.opacity = '0.85';
      stampRing.style.transform = 'rotate(-11deg) scale(1)';
    }
  }
  ...
  function rtFinalizeImage(){
    ...
    rtForceSettledState(paper);
    // Two paint frames plus a short pause so the forced styles above are
    // actually committed before html2canvas starts reading the DOM.
  ...
  document.getElementById('rtDownloadBtn').addEventListener('click', () => {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      setTimeout(() => {
        html2canvas(paper, {backgroundColor:'#F7F5EF', scale:2}).then(c => finish(c.toDataURL('image/png')));
      }, 80);
    }));
  });
