...
(function(){
  ...
  // Right before a snapshot, forces every receipt element to its fully-
  // settled end state via inline styles (not just CSS classes) — this is
  // the fix for the download coming out washed-out/blank: html2canvas can
  // capture a transition (the .rt-ink fade-in, the stamp's scale-in) before
  // the browser has actually finished painting it, even though the live
  // page already looks correct by that point.
  ...
  function rtFinalizeImage(){
    ...
    // Two paint frames so the forced styles above are actually committed
    // before html2canvas starts reading computed styles off the DOM.
